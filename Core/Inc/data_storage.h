...
/* ─── Flash page addresses (STM32G070KBT6, 128 KB flash, 2 KB pages) ───────
 * Storage now lives in the TOP 4 flash pages (60..63). The STM32G0 erases by
 * page NUMBER and programs in 64-bit doublewords (see data_storage.c).
 * Keep the linker FLASH length at 128K so application .text/.rodata never
 * grows up into this region. If you move to a different G0 density, only
 * STORAGE_FLASH_SIZE below needs changing — the four page addresses derive
 * from it.
 *
 * Page number used by HAL_FLASHEx_Erase = (addr - FLASH_BASE) / PAGE_SIZE. */
#define STORAGE_FLASH_BASE      0x08000000UL
#define STORAGE_PAGE_SIZE       0x0800UL        /* 2 KB G0 page                */
#define STORAGE_FLASH_SIZE      0x20000UL       /* 128 KB (G070KB / G071KB)    */

#define STORAGE_SETTINGS_ADDR   (STORAGE_FLASH_BASE + STORAGE_FLASH_SIZE - 4U*STORAGE_PAGE_SIZE) /* page 60 = 0x0801E000 */
#define STORAGE_LOG_A_ADDR      (STORAGE_FLASH_BASE + STORAGE_FLASH_SIZE - 3U*STORAGE_PAGE_SIZE) /* page 61 = 0x0801E800 */
#define STORAGE_LOG_B_ADDR      (STORAGE_FLASH_BASE + STORAGE_FLASH_SIZE - 2U*STORAGE_PAGE_SIZE) /* page 62 = 0x0801F000 reserved/backup */
#define STORAGE_DAILY_ADDR      (STORAGE_FLASH_BASE + STORAGE_FLASH_SIZE - 1U*STORAGE_PAGE_SIZE) /* page 63 = 0x0801F800 */

/* ─── Magic number & array sizes ────────────────────────────────────────── */
#define SETTINGS_MAGIC              0xA55A1236UL   /* v2.1: + persisted TDS/NTC calibration */
...
typedef struct {
    ...
    /* ─── v2.1 persisted sensor calibration ──────────────────────────────
     * Added after hx711_scale_x100 and before crc so the CRC region
     * (sizeof - 4) still covers them automatically. SETTINGS_MAGIC bumped
     * so any pre-v2.1 flash image is treated as blank → defaults loaded. */
    int16_t            ntc_offset_x10;    /* NTC temp calibration offset (°C×10) */
    uint8_t            tds_cal_valid;     /* 1 = TDS two-point cal below is good */
    uint8_t            _pad2;             /* align to 32-bit                   */
    int32_t            tds_cal_mv_lo;     /* TDS low  reference captured mV     */
    int32_t            tds_cal_mv_hi;     /* TDS high reference captured mV     */
    uint16_t           tds_cal_ppm_lo;    /* TDS low  reference ppm             */
    uint16_t           tds_cal_ppm_hi;    /* TDS high reference ppm             */
