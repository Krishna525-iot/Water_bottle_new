...
typedef struct {
    ...
    int16_t            offset_x10;   /**< User calibration offset (°C×10)   */
...
/**
 * @brief  Apply a fixed calibration offset to every future reading.
 * @param  hntc        Pointer to NTC handle.
 * @param  offset_x10  Offset in tenths of °C (measured+offset = true temp).
 * @note   Persisted by the app in DeviceSettings_t.ntc_offset_x10.
 */
void    NTC_SetOffset(NTC_Handle_t *hntc, int16_t offset_x10);
int16_t NTC_GetOffset(NTC_Handle_t *hntc);

/**
 * @brief  Read the UNCALIBRATED temperature (offset NOT applied).
 * @note   Used by the CALTEMP command to compute a fresh offset against a
 *         reference thermometer without the current offset skewing it.
 */
