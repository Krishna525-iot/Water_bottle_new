 ...
 * PWM frequency sets tone.  At 64 MHz, PSC=63 → 1 MHz timer clock.
 * ARR determines frequency: 1 MHz / ARR = tone Hz.
 * e.g. ARR=1000 → 1 kHz beep.
 */

#define BUZZER_TIM_CLK_HZ    1000000UL   /* after PSC=63 (64 MHz / 64 = 1 MHz) */
...
{
    s_htim = htim;
    /* Ensure PSC=63 so timer runs at 1 MHz (64 MHz core) */
